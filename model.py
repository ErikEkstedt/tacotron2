...
class Tacotron2(nn.Module):
    ...
    def forward(self, inputs):
        inputs, input_lengths, targets, max_len, output_lengths = self.parse_input(
            inputs
        )
...
class TacoGlow(nn.Module):
    def __init__(self, hparams):
        ...
        self.encoder = Encoder(hparams)
        self.decoder = GlowDecoder(hparams)

        if hparams.use_postnet:
            self.postnet = Postnet(hparams)
    ...
    def forward(self, inputs):
        ...
        nll, gate_outputs, alignments = self.decoder(
            encoder_outputs, targets, memory_lengths=input_lengths
        )
...
class GlowDecoder(Decoder):
    def __init__(self, hparams):
        ...
        glow_hparams = JsonConfig(hparams.glow_hparams)
        glow_input_size = hparams.encoder_embedding_dim + hparams.attention_rnn_dim
        self.glow = Glow(glow_hparams, glow_input_size, hparams.batch_size)
    ...
    def initialize_decoder_states(self, memory, mask):
        ...
        self.glow.initialize_nns(memory)
    ...
    def decode(self, decoder_input, mel_output=None):
        ...
        ##################################################################
        # Initialize the states of the attention rnn
        cell_input = torch.cat((decoder_input, self.attention_context), -1)
        self.attention_hidden, self.attention_cell = self.attention_rnn(
            cell_input, (self.attention_hidden, self.attention_cell)
        )
        ...
        # Calculate the attention weights and context
        attention_weights_cat = torch.cat(
            (
                self.attention_weights.unsqueeze(1),
                self.attention_weights_cum.unsqueeze(1),
    ...
    def forward(self, memory, decoder_inputs, memory_lengths):
        ...
        return nll, gate_outputs, alignments
