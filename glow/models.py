...
class FlowStep(nn.Module):
    ...
    def __init__(
        ...
        if flow_coupling == "additive":
            self.f = TacotronNN(
                in_channels // 2, in_channels, hidden_channels, condition_size
            )
    ...
    def initialize_nns(self, memory):
        self.f.initialize_state(memory)

    def forward(self, input, condition, logdet=None, reverse=False):
        if not reverse:
            return self.normal_flow(input, condition, logdet)
        else:
            return self.reverse_flow(input, condition, logdet)

    def normal_flow(self, input, condition, logdet):
    ...
    def reverse_flow(self, input, condition, logdet):
        assert input.size(1) % 2 == 0
        # 1.coupling
...
class FlowNet(nn.Module):
    ...
    def initialize_nns(self, memory):
        for l in self.layers:
            if isinstance(l, FlowStep):
                l.initialize_nns(memory)

    def forward(self, input, condition, logdet=0.0, reverse=False, eps_std=None):
        if not reverse:
            return self.encode(input, condition, logdet)
        else:
            return self.decode(input, condition, eps_std)
    ...
    def decode(self, z, condition, eps_std=None):
        for layer in reversed(self.layers):
            if isinstance(layer, modules.Split2d):
                ...
                z, logdet = layer(z, condition, logdet=0, reverse=True)
            else:
...
class Glow(nn.Module):
    ...
    def __init__(self, hparams, condition_size, batch_size):
        super().__init__()
        self.flow = FlowNet(
    ...
    def initialize_nns(self, memory):
        self.flow.initialize_nns(memory)
    ...
    def normal_flow(self, x, condition, y_onehot):
        ...
        # encode
        z, objective = self.flow(z, condition, logdet=logdet, reverse=False)
        # prior
    ...
    def initialize_nn_states(self, memory):
        pass
    ...
    def generate_attr_deltaz(self, dataset):
        ...
        with torch.no_grad():
            ...
            for i in tqdm(range(0, N, memory)):
                j = min([i + B, N])
